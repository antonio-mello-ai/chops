...
def profile(
    ...
    sample: int | None = typer.Option(
        None, "--sample", "-s", help="Sample N rows (faster for huge tables)",
    ),
) -> None:
...
def freshness(
    ...
    column: str | None = typer.Option(
        None, "--column", "-c", help="DateTime column to check (auto-detected if not provided)",
    ),
    warn_minutes: int = typer.Option(60, "--warn", help="Warning threshold in minutes"),
    critical_minutes: int = typer.Option(
        1440, "--critical", help="Critical threshold in minutes (default 24h)",
    ),
) -> None:
...
def check(
    ...
    max_null_pct: float = typer.Option(
        5.0, "--max-null-pct", help="Max null percentage before failing",
    ),
